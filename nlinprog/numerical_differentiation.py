...
def forward_difference(f: callable, h: Optional[float] = DEFAULT_H) -> callable:
    ...
    def f_prime(x: np.ndarray) -> np.ndarray:
        f_of_x = f(x)
        return h_inv * np.apply_along_axis(
            arr=np.eye(x.shape[0]), axis=0, func1d=lambda e: f(x + h * e) - f_of_x
        )
...
def central_difference(f: callable, h: Optional[float] = DEFAULT_H) -> callable:
    ...
    def f_prime(x: np.ndarray) -> np.ndarray:
        return (
            ...
            * h_inv
            * np.apply_along_axis(
                arr=np.eye(x.shape[0]),
                axis=0,
                func1d=lambda e: f(x + h * e) - f(x - h * e),
            )
        )
...
def backward_difference(f: callable, h: Optional[float] = DEFAULT_H) -> callable:
    ...
    def f_prime(x: np.ndarray) -> np.ndarray:
        f_of_x = f(x)
        return h_inv * np.apply_along_axis(
            arr=np.eye(x.shape[0]), axis=0, func1d=lambda e: f_of_x - f(x - h * e)
        )
