...
class SufficientDecreaseCondition:
    ...
    def __call__(self, alpha: float) -> bool:
        return np.all(
            self.phi(alpha)
            <= self.phi_of_zero + self.c1 * alpha * self.phi_prime_of_zero
        )
...
class CurvatureCondition:
    ...
    def __call__(self, alpha: float) -> bool:
        return np.all(
            self.phi_prime(alpha * np.ones(1)) >= self.c2 * self.phi_prime_of_zero
        )
...
class ArmijoBacktraackingLineSearch(LineSearch):
    ...
    def __call__(self, f: callable, x_k: np.ndarray, d_k: np.ndarray) -> float:
        ...
        armijo_condition = SufficientDecreaseCondition(
            phi=phi, c1=self.epsilon * self.eta
        )
...
class WolfeZoomLineSearch(LineSearch):
    def __init__(
        self, alpha_max: float = 2.0, c1: float = 1e-4, c2: float = 0.9, *args, **kwargs
    ):
    ...
    def __call__(self, f: callable, x_k: np.ndarray, d_k: np.ndarray) -> float:
        ...
        while True:
            ...
            if not sufficient_decrease_condition(alpha_i) or (
                phi_of_alpha_i > phi_of_alpha_i_minus_one and i > 1
            ):
            ...
            if i > self._early_termination_iters:
                return alpha_i  # early termination TODO: make better

    def _zoom(
        self, phi: callable, phi_prime: callable, alpha_low: float, alpha_high: float
    ) -> float:
        ...
        while True:
            ...
            if (phi_of_alpha > phi_of_zero + self.c1 * alpha * phi_prime_of_zero) or (
                phi_of_alpha > phi(alpha_low)
            ):
                alpha_high = alpha
            else:
...
LINE_SEARCH_MAPPING = MappingProxyType(
    {"armijo": ArmijoBacktraackingLineSearch, "wolfe": WolfeZoomLineSearch}
)
