...
def calc_dfp_inverse_hessian(H_k: np.ndarray, p_k: np.ndarray, q_k: np.ndarray, *args, **kwargs) -> np.ndarray:
    ...
    H_k_plus_one = np.array(H_k)
    H_k_plus_one += np.divide(np.outer(p_k, p_k), (q_k @ q_k.T))
    H_k_plus_one -= np.divide(np.outer(H_k @ q_k,  H_k @ q_k), (q_k.T @ H_k @ q_k))
    return H_k_plus_one
...
def calc_bfgs_inverse_hessian(H_k: np.ndarray, p_k: np.ndarray, q_k: np.ndarray, *args, **kwargs) -> np.ndarray:
    ...
    # return H_k_plus_one
    # H_k_plus_one = np.array(H_k)
    # H_k_plus_one += ((1 + q_k.T @ H_k @ q_k) / (p_k.T @ q_k)) * (np.outer(p_k, p_k) / (p_k.T @ q_k))
    # H_k_plus_one -= (np.outer(p_k, q_k) @ H_k + H_k @ np.outer(q_k, p_k)) / (q_k.T @ p_k)
    # return H_k_plus_one
    return calc_broyden_inverse_hessian(H_k=H_k, p_k=p_k, q_k=q_k, phi=1.0)
