...
def steepest_descent(f: callable, x0: np.ndarray, atol:Optional[float]=None, rtol:Optional[float]=None, maxitters: Optional[int]=100) -> np.ndarray:
    ...
    for k in range(maxitters):
        d_k = -1*grad(x_k)
        alpha_k = armijo_line_search(f, x_k, d_k)
        u_k = alpha_k * d_k
        x_k += u_k
