...
class NumericalDerivativeTestFunction():
    def __init__(self, f:callable, x: np.ndarray, f_jacobian: np.ndarray, f_hessian: np.ndarray) -> None:
        self.f = f
        self.x = x
...
@pytest.mark.parametrize("jacobian", [forward_difference, backward_difference, central_difference])
